...
def find_and_click(template_img, threshold=0.7, show=False):
    global w, h
    ...
    for scale in scales:
        try:
            if scale == 1.0:
                ...
                tpl = cv2.resize(template_img, (new_w, new_h), interpolation=cv2.INTER_AREA if scale < 1.0 else cv2.INTER_CUBIC)
...
try:
    while True:
        if retry_template is not None:
            ...
            found = find_and_click(retry_template, threshold=0.7, show=False)
            if found:
                time.sleep(2)
                print("Checking for upgrade bar button...")
                find_and_click(upgrade_bar_template, threshold=0.6, show=False)
                time.sleep(1)
        ...
        if health_hack_template is not None:
            ...
            find_and_click(health_hack_template, threshold=0.7, show=False)
            time.sleep(0.1)
            # check for gem claim
            find_and_click(gem_claim_template, threshold=0.7, show=False)
        time.sleep(3)
